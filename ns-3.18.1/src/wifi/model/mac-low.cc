                           ...
                           MacLowTransmissionListener *listener)
{
  ...
  if (m_txParams.MustSendRts ())
  ...
  else
    {
     ...
     if (NeedCtsToSelf(dataTxVector) && m_ctsToSelfSupported)
        {
          SendCtsToSelf();
        }
      else
        {
          SendDataPacket (dataTxVector);
       }
    }
...
bool
MacLow::NeedCtsToSelf (WifiTxVector dataTxVector)
{
	//jychoi
  //WifiTxVector dataTxVector = GetDataTxVector (m_currentPacket, &m_currentHdr);
...
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiMode txMode, WifiPreamble preamble)
{
  ...
  if (hdr.IsRts ())
           ...
           && m_txParams.MustWaitAck ())
    {
      ...
      m_stationManager->ReportRxOk (m_currentHdr.GetAddr1 (), &m_currentHdr,
                                    rxSnr, txMode);
			// return rxSnr when received Ack
  ...
  else if (hdr.GetAddr1 () == m_self) //jychoi
    {
      m_stationManager->ReportRxOk (hdr.GetAddr2 (), &hdr,
                                    rxSnr, txMode);
...
void
MacLow::SendDataPacket (WifiTxVector dataTxVector)
{
  NS_LOG_FUNCTION (this);
